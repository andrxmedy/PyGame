...
class Level1:
    def __init__(self, game):
        ...
        # Carregar e ajustar a imagem do botão de voltar
        ...
        self.perspective_image = pygame.image.load('Fundo Fases/swamp-overlay.png')
        self.perspective_image = pygame.transform.scale(self.perspective_image, (WINDOW_WIDTH, WINDOW_HEIGHT))
        ...
        self.enemies = [
            ...
            Enemy(300, 300, 50, 50, 'vespa', dificuldade=1),
            Enemy(300, 300, 50, 50, 'vespa', dificuldade=2),
            Enemy(500, 100, 50, 50, 'mosca', dificuldade=1),
            Enemy(500, 100, 50, 50, 'besouro', dificuldade=2),
        ]
    ...
    def verificar_colisao(self):
        for enemy in self.enemies:
            if enemy.rect.colliderect(self.player.rect):
                ...
                rel_y = ponto_de_colisao.y - self.player.rect.y

                if self.player.atacando:
                    if (self.player.direcao == 'direita' and rel_x > self.player.rect.width / 2) or \
                            (self.player.direcao == 'esquerda' and rel_x < self.player.rect.width / 2):
    ...
    def update(self):
        ...
        if self.player.vida <= 0:
            ...
            self.game.change_state(Defeat(self.game, 1))

        if not self.enemies:
            from states.victory import Victory
            self.perfil_jogador.niveis_completados['nivel1'] = True
            self.perfil_jogador.salvar_progresso()
            self.game.change_state(Victory(self.game, 1))
    ...
    def draw(self):
        ...
        self.game.screen.blit(self.perspective_image, (0, 0))
        pygame.display.flip()
